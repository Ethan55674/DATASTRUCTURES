...
void BST<T>::deleteLeaf(BTNode<T>* child, BTNode<T>* parent){
    if (isFullTree() || parent -> right != nullptr || parent -> left != nullptr){
        return;
    }
    else {
        if (root == child){
...
template <typename T>
void BST::print( const std::string& prefix, BTNode<T> node, bool isRight) const{
    if (!node){
        return;
    }
    std::cout << prefix;
